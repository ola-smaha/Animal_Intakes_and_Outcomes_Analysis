...
import pandas as pd
import numpy as np
from lookups import TransformationErrors, IntakesOutcomesTablesNames, Open_AI
from logging_handler import log_error_msg
import warnings
import json
from data_extraction_handler import *
from openai_handler import get_gpt_animal_type_string
...
def edit_all_animal_types(dfs):
    try:
        bird_types = get_gpt_animal_type_string(content_name = Open_AI.BIRD_TYPE, content= Open_AI.GPT_BIRD_CONTENT)
        livestock_type = get_gpt_animal_type_string(content_name = Open_AI.LIVESTOCK_TYPE, content= Open_AI.GPT_LIVESTOCK_CONTENT)
        birds_list = bird_types.split(', ')
        birds_list = [i.title() for i in birds_list]
        livestock_list = livestock_type.split(', ')
    ...
    try:
        ...
        for df in intakes_dfs:
            ...
            df['breed'] = df['breed'].replace({
                ...
                'Anatol ':'Anatolian ', 'Eng ': 'English ', 'Retriever':'Retr', 'Min':'Miniature', 'Pit Bull': 'Pitbull'},regex=True)
        ...
        edit_all_animal_types(intakes_dfs)
        for df in intakes_dfs:
