...
def clean_sonoma_dataset(dfs):
    ...
    # sonoma[['date_of_birth','outcome_date']] = sonoma[['date_of_birth','outcome_date']].apply(lambda x: x.fillna(pd.NaT))
...
def clean_all_data(limit):
    ...
    for key,value in dfs.items():
        if not key.startswith("shelter"): # sufficiently big enough (30 years)
            if len(value)<30:
